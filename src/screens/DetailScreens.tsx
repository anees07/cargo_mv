...
export function CreateTripScreen() {
  ...
  const handleCreate = () => {
    ...
    selectTrip(trip.id);
    toast({ title: "Trip created", body: `${trip.tripNumber} ready — open it to start loading.`, variant: "success" });
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <Card className="border-dashed p-3 text-xs text-slate-600">
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="font-semibold text-slate-900">Trip number will be generated by the server</p>
              <p className="mt-0.5">The numbering sequence is locked per business profile. Multi-device conflicts are prevented by PostgreSQL advisory locks in the InsForge Edge Function.</p>
            </div>
          </div>
        </Card>
      </div>
