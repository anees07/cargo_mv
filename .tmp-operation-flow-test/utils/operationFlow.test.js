...
import { buildOffloadAvailability, billTypeForOperationType, isOperationBillable, validatePaymentRequest, } from "./operationFlow.js";
...
test("bill generation is blocked when an active draft bill already exists for the operation", () => {
    ...
    assert.equal(isOperationBillable(loadingOperation, [trip], [bill({ billType: billTypeForOperationType(loadingOperation.operationType) })]), false);
});
test("cancelled bills do not block rebilling the same operation", () => {
    const loadingOperation = operation({ items: [item({})] });
    assert.equal(isOperationBillable(loadingOperation, [trip], [bill({ billStatus: "cancelled" })]), true);
});
